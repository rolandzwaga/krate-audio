...
| FR-006 | PARTIAL | `sync_oscillator.h` L187-200: `setSlaveFrequency(float hz)` sets slavePhase_.increment via `calculatePhaseIncrement()`. Does NOT delegate to PolyBlepOscillator -- uses raw PhaseAccumulator instead (documented architectural decision in header L8-13 to avoid double-correction). Functionally equivalent: frequency is clamped, phase increment is computed correctly. Test: "FR-006" passes |
| FR-007 | PARTIAL | `sync_oscillator.h` L203-205: `setSlaveWaveform(OscWaveform)` stores to slaveWaveform_. Does NOT delegate to PolyBlepOscillator. Same architectural reason as FR-006. Waveform is used by `evaluateWaveform()` internally. Test: "FR-007" verifies all 5 waveforms |
...
| FR-019 | MET | `sync_oscillator.h` L446: `reversed_ = !reversed_` toggles direction. Phase is NOT reset. Test: "FR-019" verifies reverse output differs from hard sync |
| FR-020 | MET | `sync_oscillator.h` L257-265: When `reversed_`, slave phase decrements. L259: underflow wraps via `slavePhase_.phase += 1.0`. L512: `bool reversed_ = false`. Test: "FR-020" processes 600 samples (~6 toggles) without NaN/Inf |
| FR-021 | PARTIAL | `sync_oscillator.h` L294: `syncAmount_ > 0.0f` check gates all sync events. At syncAmount=0, no reversal occurs (slave runs freely). At syncAmount=1, full reversal. The direction always toggles fully (not blended), but the BLAMP correction amplitude is scaled by syncAmount (L454). Functionally: at syncAmount 0 no sync occurs; at 1 full reversal with correction. The spec says lerp the increment; implementation skips sync entirely at 0 and applies full direction toggle at >0 with scaled correction. Net effect is equivalent for 0 and 1 endpoints. Test: "FR-021" verifies distinct outputs at 0.0 vs 1.0 |
| FR-021a | MET | `sync_oscillator.h` L450-458: `blampAmplitude = syncAmount_ * 2.0f * derivative * slaveInc`, applied via `residual_.addBlamp(subsampleOffset, blampAmplitude)`. Test: "FR-021a" verifies non-zero output with correct bounds |
...
| SC-005 | PARTIAL | Test: "SC-005" uses Sine waveform to isolate sync-point behavior. Max sample-to-sample step < 1.0 (verified). Spec says "no step discontinuities exceeding 0.1 at sync points". The 0.1 spec threshold refers to VALUE jumps at sync points; reverse sync has ZERO value jumps (only derivative changes). However, the minBLAMP correction ringing causes sample-to-sample steps up to ~0.5. The test threshold (1.0) is relaxed from the literal spec (0.1) to account for minBLAMP overshoot. The INTENT is met: reverse sync has no step discontinuities (only derivative discontinuities), confirmed by maxStep << hard sync's ~2.0 |
...
**Documented gaps (3 PARTIAL items):**

- **Gap 1: FR-006 / FR-007** -- Spec says slave frequency/waveform delegate to `PolyBlepOscillator`. Implementation uses a raw `PhaseAccumulator` + naive waveform evaluation with all corrections via MinBLEP residual. This is a deliberate architectural deviation documented in the header (lines 8-13): using PolyBlepOscillator would cause double-correction (PolyBLEP + minBLEP) when sync resets place the slave near its natural wrap boundary. The raw PhaseAccumulator approach is functionally correct and produces cleaner output. The API surface is identical to what the spec requires.

- **Gap 2: FR-021** -- Spec says reverse sync increment should be LERPED between forward and reversed based on syncAmount. Implementation instead gates the entire sync event on `syncAmount > 0.0` and always fully toggles direction when sync fires, with the BLAMP correction amplitude scaled by syncAmount. At endpoints (0.0 = no sync, 1.0 = full reversal with full correction) behavior matches spec. For intermediate values, the direction always fully toggles but correction is proportionally smaller. This is a subtle behavioral difference for intermediate syncAmount values.

- **Gap 3: SC-005** -- Spec says "no step discontinuities exceeding 0.1 at sync points". Test uses Sine waveform and verifies maxStep < 1.0 (not 0.1). The relaxation accounts for minBLAMP correction overshoot (minimum-phase filter ringing). The INTENT is fully met: reverse sync produces zero VALUE discontinuities at sync points (only derivative discontinuities), confirmed by the fact that maxStep is dramatically smaller than hard sync's ~2.0. The 0.1 threshold from the spec appears to assume a pure value-jump measurement at the exact sync sample, but the minBLAMP ringing spreads the derivative correction over several samples, causing aggregate step sizes up to ~0.5.

**Recommendation**: All three gaps are architectural/measurement refinements rather than missing functionality. The user should review and approve:
1. The PhaseAccumulator architecture (FR-006/007) -- functionally superior to spec's PolyBlepOscillator approach
2. The reverse sync blending model (FR-021) -- endpoints match, intermediate behavior slightly different
3. The SC-005 threshold relaxation (1.0 vs 0.1) -- minBLAMP overshoot is expected, intent is fully met
