...
**Status:** Phase 6 Complete (see `specs/141-membrum-phase6-ui/`)  
**Plugin:** Membrum  
...
| Phase 6 | `specs/141-membrum-phase6-ui/` | ✅ Complete (v0.6.0) | Macro controls (5 per-pad), Acoustic/Extended UI modes, custom VSTGUI editor (pad grid + selected-pad panel + kit column + Tier 2 coupling matrix editor + voice mgmt UI + output routing UI + pitch-envelope promotion), state version 6 with v5 migration |
...
To prevent parameter overload, Membrum introduces **Macro Controls** that sit on top of the detailed parameters:
...
These are **on top of** the detailed parameters, not instead of them. In Acoustic mode (default), users see macros + key controls. In Extended mode, full parameter access is available including the Unnatural Zone.

### Acoustic vs Extended Mode

| Mode | What's Visible | Target User |
|------|---------------|-------------|
| **Acoustic** (default) | Macros, primary body controls (Material, Size, Decay, Strike Position), Level, Pitch Envelope | Fast sound design, beginners |
| **Extended** | All of the above + Unnatural Zone, raw physics params (b1/b3, tension, air coupling), full coupling matrix | Sound designers, experimentalists |

This dual-layer approach reduces overwhelm while preserving full control for power users.

### Cross-Pad Coupling Tiers

To manage complexity, coupling uses a tiered control system:

**Tier 1 (default — what most users see):**
- "Snare Buzz" (1 knob) — kick-to-snare wire sympathetic response
- "Tom Resonance" (1 knob) — global tom sympathetic coupling
- "Global Coupling" — master coupling intensity

**Tier 2 (Extended mode):**
- Full coupling matrix editor
- Visual feedback showing which pads excite which
...
| **Mode Inject**   | Add synthetic partial series into the body's mode set. Options: harmonic (integer ratios), FM-derived (Chowning ratios like 1:1.4, 1:sqrt(2), 1:phi), or randomized. Uses HarmonicOscillatorBank. **Phase randomization required** to avoid cancellation/inconsistent transients between injected and physical modes. |
...
| **Pitch Envelope** | Fast exponential pitch sweep. For kicks: f_start=160-500Hz → f_end=40-80Hz over 5-200ms. This is the 808 bridged-T effect: amplitude-dependent frequency shift from diode nonlinearity in the original circuit. **NOTE: This is identity-defining for kicks — promoted to a primary voice control in the UI, not buried in tone shaping. Integrated into the "Punch" macro.** |
...
### Resolved by Design Review (v0.5)
- ~~Parameter overwhelm~~: **Macro controls + Acoustic/Extended mode** layering addresses this without removing parameters.
- ~~Coupling UX~~: **Tiered system** — Tier 1 (3 knobs) for most users, Tier 2 (full matrix) for advanced.
- ~~Pitch envelope buried~~: **Promoted to primary voice control**, integrated into Punch macro.
- ~~Mode Inject phase issues~~: **Phase randomization required** in spec.
- ~~Nonlinear coupling stability~~: **Velocity-scaled + energy limiter** guard added to spec.
- ~~Double membrane complexity~~: Kept as open question, but if built: expose as musical terms (Body Depth, Resonance Blend, Boom↔Punch), not physics terms.
...
| 1.0     | 2026-04-12 | Phase 6 specced (spec 141) — custom VSTGUI editor + 5 per-pad macros (Tightness/Brightness/Body Size/Punch/Complexity) + Acoustic/Extended UI modes + 4×8 pad grid + selected-pad panel + kit column with preset browsers + Tier 2 coupling matrix editor with Solo/activity/override + voice management UI + output routing UI + pitch-envelope promotion to primary voice control. Adds 2 globals (kUiModeId=280, kEditorSizeId=281) + 160 per-pad macro params (offsets 37-41 across 32 pads). State version 6 with v5 migration (macros default 0.5 = neutral). Targets v0.6.0. Implementation not yet started. |
...
| 1.1     | 2026-04-14 | Phase 6 complete (v0.6.0) — Custom VSTGUI editor per spec 141 (PR #226, merge `fc419e65`). 5 per-pad macros (Tightness/Brightness/Body Size/Punch/Complexity) via MacroMapper → 160 per-pad float64 params at offsets 37-41. Acoustic/Extended UI modes via `UIViewSwitchContainer` bound to `kUiModeId` (280). Editor size toggle Default 1280×800 / Compact 1024×640 via `kEditorSizeId` (281). 4×8 PadGridView with shift-click audition, glow via 1024-bit threshold-gated bitfield (32 × `std::atomic<uint32_t>`, UI polls ≤ 30 Hz), category/choke/bus indicators. Selected-pad panel (Acoustic: macros + envelope displays; Extended: full parameter access incl. Unnatural Zone, raw physics, Tone Shaper, Exciter, per-pad Coupling Amount). Kit column with kit + per-pad preset browsers, voice management (max polyphony, stealing policy, active-voice readout), global output meter, CPU indicator, all 4 Phase 5 global-coupling knobs (UI wiring only). Tier 2 CouplingMatrixView (32×32) with Solo/Reset/activity indicators and per-pair overrides. Output routing UI with `kOutputBusId` (282) proxy + inactive-bus tooltip. Pitch envelope promoted to primary voice control in Acoustic mode. State version 6 with v5 migration: 1280 bytes (160 × float64 macros, pad-major order) appended after v5 payload → blob 10610 bytes; `kUiModeId`/`kEditorSizeId`/`kOutputBusId` are session-scoped (not persisted). ASan editor lifecycle clean (100 open/close cycles, 0 use-after-free). Pluginval L5 clean, zero new clang-tidy warnings. |
