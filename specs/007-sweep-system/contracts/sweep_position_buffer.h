// ==============================================================================
// API Contract: SweepPositionBuffer
// ==============================================================================
...
// Reference: specs/Disrumpo/custom-controls.md Section 1.3.4
// ==============================================================================
...
public:
    ...
    /// oldest entry is overwritten (newest data preferred for UI).
    ///
    /// @param data Sweep position data to push
    /// @return true if pushed successfully, false if buffer was full (data lost)
    bool push(const SweepPositionData& data) noexcept {
        ...
        if (currentCount >= kSweepBufferSize) {
            // Buffer full - UI not keeping up, drop oldest
            // In practice, UI should read faster than audio writes
            return false;
        }
    ...
    /// @param data Output parameter for retrieved data
    /// @return true if data was available, false if buffer was empty
    bool getLatest(SweepPositionData& data) const noexcept {
        const int currentCount = elementCount_.load(std::memory_order_acquire);
        if (currentCount == 0) {
            return false;  // Empty
        }
    ...
    SweepPositionData getInterpolatedPosition(uint64_t targetSample) const noexcept {
        const int currentCount = elementCount_.load(std::memory_order_acquire);
        if (currentCount == 0) {
