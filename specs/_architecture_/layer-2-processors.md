...
**Sequencer Note lane (Spec 142, Gradus only):**

Lane 10 — Sequencer Note — is the only Gradus-exclusive addition to `ArpeggiatorCore`. It bumps `kNumLanes` from 9 to 10 and is *conditionally inert* in Live mode so that Ruinae and pre-feature Gradus presets continue to produce byte-identical MIDI (SC-004, SC-004b).

- New enum: `enum class SourceMode : uint8_t { Live, Sequencer }` (default `Live`). Toggle via `void setSourceMode(SourceMode mode) noexcept`.
- New lane storage: `ArpLane<uint8_t> seqNoteLane_` holds 32 per-step MIDI pitches (defaults 60), plus a parallel `std::array<std::atomic<uint8_t>, 32> seqRestFlags_` (defaults 1 = rest) because `ArpLane` carries one value per step and the rest bit needs to live alongside it without changing the existing template.
- **Conditional-inert branch**: in `fireStep()` the lane-10 advance call is wrapped in `if (sourceMode_ == SourceMode::Sequencer) { advanceLaneBySpeed(seqNoteLane_, 9); }`. In Live mode the call is skipped entirely — `seqNoteLane_.currentStep()` stays at 0, no read of `seqRestFlags_` ever occurs, and emission falls through the unchanged held-note selector. This is what guarantees SC-004/SC-004b: Ruinae and v2 Gradus state never enter Seq mode, never touch the new lane, and produce bit-identical output.
- **Source-mode-aware `noteOff`**: in Seq mode `noteOff()` bypasses `LatchMode` entirely (`heldNotes_.remove(note); return;`) so that the transposition root reverts on physical key release regardless of `LatchMode::Hold` / `LatchMode::Add`. This is FR-022 (Latch ignored in Seq mode) implemented as a single early branch in the core rather than at the plugin level — Ruinae cannot accidentally toggle Latch behavior because it never reaches Seq mode.
- **Per-stage pitch clamp**: in Seq mode `fireStep()` computes `transposedPitch = clamp(programmedPitch + (heldRoot - 60), 0, 127)` where `heldRoot = heldNotes_.empty() ? 60 : heldNotes_.byInsertOrder().back().note`. This intermediate clamp matches Live mode's behavior — the existing pitch lane and global `kArpTranspose` stages each apply their own `[0,127]` clamp downstream, so the cumulative result is equivalent to a single combined clamp on the final emitted pitch (FR-024).
- **Base velocity**: `baseVelocity = heldNotes_.empty() ? 100 : heldNotes_.byInsertOrder().back().velocity` (FR-025a). The Velocity lane scales on top exactly as in Live mode (FR-020).
- **Lane-bounds audit (grilling-pass)**: the `kNumLanes` 9->10 bump also fixed a pre-existing bug at `consumePendingCurveTables` which hardcoded `for (size_t i = 0; i < 8; ++i)` and silently skipped lane 8's (MIDI delay) curve table. All per-lane storage arrays (`laneSpeedMultipliers_`, `laneSpeedCurveTables_`, swing/jitter arrays, retrigger reset loops) are now sized and iterated by `kNumLanes`.
