...
2 % ceiling is derived rather than guessed.

> **AMENDED 2026-09-14 after measurement (spec D-15). The table above is an upper bound on the phase
> *step*, NOT a prediction of measured THD, and the phase reset is not the dominant term.** Measured
> on the shipped build, the backed-out phase-reset contribution is ≈ **0.007 %** at `Div2 @ 220 Hz`
> — ≈ 200× under this table's 1.44 %, because the bound prices the worst-case step as if it landed
> inside `k = 2..10`, while the reset happens *at the sine zero crossing* (phase jitter, not an
> amplitude step) and its energy spreads over the whole band. What SC-004 measures is ≈ 100 % third
> harmonic (H3 = 0.0841 % against ≈ 0.0008 % each for H2 and H4…H10) from FR-041's always-in-circuit
> `tanh` — memoryless and odd, so its relative harmonic content is frequency-independent — tilted by
> the FR-042 18 Hz Bessel high-pass. Consequently the sentence this section used to end with — that
> SC-004 (b) asserts the trend is monotonically non-increasing because the discontinuity is strictly
> proportional to `masterInc` — **is withdrawn**: the raw figure *rises* as `f` descends on all three
> tones (0.0842 → 0.0871 → 0.1024 % for `Div2`), and D-4's pre-authorised lever would remove the only
> term that trends with `f` at all, making that clause *more* false. **SC-004 (b) now asserts
> flatness of the FR-042-corrected THD within each sweep (spread ≤ 4 % of mean, with a raw-spread
> ≥ 5 % non-vacuity floor); see spec.md SC-004 (b) and D-15.** The 2 % ceiling of SC-004 (a) is
> untouched, and if measurement ever disagrees with *it*, FR-071's stop-and-surface rule applies: the
> pre-authorised lever is D-4's rejected alternative (a private `PhaseAccumulator` + `std::sin` for
> the Sine path), **never** a relaxed ceiling.
...
| **SC-002 (a)–(e)** the tracking law | `..._spectral_test.cpp` / `SubharmonicEngine_TrackingLaw` | isolated fixture **except** (and **including the gate**, which SC-002 replaces with S10.3's two-part anchor gate — the unconditional −60 dBFS floor would fail this case at the −60 dBFS body on a correct implementation, predicted tap −64.5 dBFS): tracking 1.0 (the quantity under test), all three tones at their FR-020 defaults, breath depths 0, `f = 55`, input a mono-identical 55 Hz sine held 8 s per step with the last 4 s measured. Sweep body RMS −60/−48/−36/−24/−18/−12/−6 dBFS. Measure `subTap` RMS. (a) unity slope in dB, ±1.0 dB, across −60…−24 (the −18 point is already past the knee: the RMS sensor over-reads a steady sine by ≈ 1.9 dB at the FR-031 constants, so the knee is at reference − 1.9 dB; ruled 2026-09-13); (b) flat ±0.5 dB across −18…−6; (c) with tracking 0, flat ±0.5 dB across the whole sweep; (d) `getTrackingGain()` agrees with `(1−a) + a·getTrackedEnvelope()` within `max(1e-4, 2 %)` (the sensor's ≈ 1.5 % 2f ripple is averaged by the ramp but not by the getter; ruled 2026-09-13); (e) the knee lies at reference − 1.9 dB (the recorded sensor bias) ±1.0 dB at both the default reference and `setTrackReferenceDb(-30)`, and moves by −12 dB ±1.0 dB between them (the −24 point becomes flat at the −30 reference; ruled 2026-09-13). **Fixture note:** the follower is RMS-mode, so a sine of peak `A` reads `A/√2` — the test computes the body's actual RMS from the rendered buffer rather than assuming a dBFS convention. |
...
| **SC-004 (a)–(b)** divider THD | `..._spectral_test.cpp` / `SubharmonicEngine_DividerTHD` | same fixture and sweeps. `measureLowFrequencyThdPercent(..., maxHarmonic = 10)`. (a) ≤ **2.0 %** for every tone at every fundamental, every value transcribed into compliance; (b) **amended 2026-09-14 (spec D-15)** — the FR-042-corrected THD is **flat within each tone's own** descending sweep (220→110→55 for Div2/Fifth; 440→220→110 for Div4), spread **≤ 4 % of the sweep mean**, with the **raw** spread required to **exceed 5 %** so the correction stays load-bearing; never compared across tones. The earlier "monotonically non-increasing" form is withdrawn — measured, the raw figure rises as `f` descends on all three tones, and the whole rise is the FR-042 tilt this clause divides out. S4.2 bounds the phase *step*, not the measured THD. A negative sentinel FAILS the case. |
...
| **SC-016** shared components green + unchanged | not a `TEST_CASE` | **Amended 2026-09-14 by ruling (SC-013 (c)): `sub_oscillator.h` gains an append-only `advance()`; its diff must be exactly that method, and `sub_oscillator_test.cpp` gains the `advance()`/`process()` equivalence case.** `git diff --stat` over the ten FR-080 headers is **empty**; `dsp_core_tests`, `dsp_primitives_tests`, `dsp_processors_tests` (in particular `sub_oscillator_test.cpp`) and the `seraphis_*` cases inside `dsp_systems_tests` all pass. |
