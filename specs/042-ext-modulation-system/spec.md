...
- **FR-023**: Changes to global modulation routing amounts MUST be smoothed using the existing OnePoleSmoother (120 ms time constant, meeting SC-003's -60 dBFS per-sample step threshold) to prevent zipper noise. Amount changes on already-active routes use `setTarget()` for smooth transitions; new route activations use `snapTo()` for immediate response. Per-voice route amount changes do NOT require smoothing and are applied instantly at the next block boundary (not mid-block), as the per-voice sources (envelopes, LFOs) already change smoothly and the VoiceModRouter is designed for minimal overhead.
...
| FR-023 | MET | `modulation_engine.h` line 111: `smoother.configure(120.0f, ...)` -- OnePoleSmoother with 120ms time constant for global route amounts. `evaluateRoutings()`: captures amountStart (pre-advance) and amountEnd (post-advance) per routing, computes start/end offset pairs via `applyBipolarModulation()`. `getModulationOffsetInterp()` exposes pairs for consumer-side per-sample interpolation. `setRouting()` (line 293): uses `setTarget()` for active routes (smooth transition) and `snapTo()` for new routes only. Per-voice amounts are instant (no smoothing in VoiceModRouter). |
...
| SC-003 | MET | Tests at **block_size=512** (real-world conditions): "SC-003: Amount change 0->1 at block_size=512 produces smooth interpolation", "SC-003: Amount change 1->0 at block_size=512 produces smooth ramp-down", "SC-003: Multiple simultaneous amount changes at block_size=512 all smooth" (ext_modulation_test.cpp). Uses `getModulationOffsetInterp()` API which exposes start-of-block and end-of-block offset pairs. Per-sample step = \|end-start\|/512; worst case with 120ms smoother: 0.38/512 = 0.000749 (-62.5 dBFS), below -60 dBFS threshold. Tests verify: (1) per-sample step < 0.001 for all blocks, (2) block boundary continuity (start[N+1] == end[N]), (3) convergence within 50 blocks. Implementation: `evaluateRoutings()` computes both start (pre-smoother-advance) and end (post-smoother-advance) contributions. `modOffsets_` set to end values for backward compatibility. Per-voice amounts instant by design (FR-023). |
