...
| FR-023 | MET | `modulation_engine.h` line 111: `smoother.configure(120.0f, ...)` -- OnePoleSmoother with 120ms time constant for global route amounts (increased from 20ms to meet SC-003 -60 dBFS threshold). Lines 638-641: `amountSmoothers_[i].setTarget()` then `advanceSamples(numSamples)` for correct per-block advancement. `setRouting()` (line 286): uses `setTarget()` for active routes (smooth transition) and `snapTo()` for new routes only. Per-voice amounts are instant (no smoothing in VoiceModRouter). |
...
| SC-003 | MET | Tests: "SC-003: Global route amount change 0->1 produces smooth transition", "SC-003: Global route amount change 1->0 produces smooth ramp-down", "SC-003: Multiple simultaneous amount changes all smooth" (ext_modulation_test.cpp lines 503-614). Measured: max per-sample step = 0.000944 (-60.5 dBFS), below -60 dBFS threshold (0.001). Convergence: 4875 samples (~110ms) to 99%. Fixes applied: (1) `setRouting()` uses `setTarget()` for active routes instead of `snapTo()` to enable actual smoothing; (2) smoother time increased from 20ms to 120ms to meet -60 dBFS per-sample step; (3) `advanceSamples(numSamples)` added to OnePoleSmoother for correct per-block advancement. Per-voice amounts instant by design (FR-023). |
