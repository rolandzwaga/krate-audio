...
- **SC-005**: With smoothing time of 10 ms at 44.1 kHz, after an abrupt X change from -1.0 to +1.0, the smoothed X value MUST reach within 5% of +1.0 within 50 ms (approximately 5 time constants), verified by processing 2205 samples (50 ms) and checking the final smoothed value.
...
| FR-026 | MET | `vector_mixer.h` L195-197: targetX_, targetY_, smoothingTimeMs_ are `std::atomic<float>`. L259,263,267,268,281: All use `memory_order_relaxed`. L208-209: topology_, mixingLaw_ are plain members (not thread-safe, documented). Test: "atomic setters are thread-safe" (L1110). |
