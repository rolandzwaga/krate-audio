...
|-------------|--------|----------|
| FR-001 | MET | `ruinae_effects_chain.h` line 65: `class RuinaeEffectsChain` at Layer 3, composes DigitalDelay/TapeDelay/PingPongDelay/GranularDelay/SpectralDelay/FreezeMode/Reverb (all Layer 4). Fixed order: Freeze->Delay->Reverb (processChunk lines 355-446). Test: "prepare/reset lifecycle" passes. |
...
| FR-003 | MET | `ruinae_effects_chain.h` line 150: `void reset() noexcept` -- resets all delays, freeze, reverb, comp delays, crossfade state, freezeFadeRemaining_. Test: "prepare/reset lifecycle - reset after prepare does not crash" passes. |
...
| FR-005 | MET | processChunk lines 367 (freeze), 378 (delay+crossfade), 445 (reverb). Test: "FR-005: fixed processing order" -- impulse with 200ms delay: early energy=0.0, late energy=0.386. Late >> early confirms delay runs before reverb. |
...
| FR-008 | MET | `ruinae_effects_chain.h` line 587: `RuinaeDelayType activeDelayType_ = RuinaeDelayType::Digital`. Test: "prepare/reset lifecycle - default delay type is Digital" passes. |
| FR-009 | MET | `ruinae_effects_chain.h` line 220: `void setDelayType(...)` -- initiates crossfade when type differs. Test: "FR-009: setDelayType selects active delay" passes (Tape and Spectral verified). |
| FR-010 | MET | `ruinae_effects_chain.h` lines 385-423: outgoing into crossfadeOut buffers, incoming into left/right, per-sample blend `left[i] = crossfadeOutL_[i] * (1-alpha) + left[i] * alpha`. Test: "FR-010: crossfade blends outgoing and incoming" passes. |
| FR-011 | MET | `crossfadeIncrement(kCrossfadeDurationMs=30.0f, sampleRate_)`. Measured: 30.48 ms (1344 samples at 44.1kHz). Test: "FR-011: crossfade duration 25-50ms" passes. |
| FR-012 | MET | `ruinae_effects_chain.h` lines 226-228: `if (crossfading_) { completeCrossfade(); }`. Test: "FR-012: fast-track" passes (Digital->Tape mid-fade, then Tape->Granular, final type is Granular). |
...
| FR-014 | MET | `ruinae_effects_chain.h` line 222: `if (!crossfading_ && type == activeDelayType_) return;`. Test: "FR-014: same type is no-op" passes. |
| FR-015 | MET | `ruinae_effects_chain.h` lines 253-279: setDelayTime/setDelayFeedback/setDelayMix forward to all 5 types. Test: "FR-015: delay parameter forwarding" passes. |
...
| FR-018 | MET | `ruinae_effects_chain.h` lines 294-326: setFreezeEnabled (with fade-out logic), setFreeze, setFreezePitchSemitones, setFreezeShimmerMix, setFreezeDecay -- all noexcept. Tests: "FR-018: setFreezeEnabled" and "FR-018: freeze parameter forwarding" pass. |
| FR-019 | MET | processChunk line 367: `if (freezeEnabled_ || freezeFadeRemaining_ > 0) freeze_.process(...)`. Test: "FR-019: freeze captures and holds spectrum" -- frozen RMS > 0.001 after feeding silence. |
...
| FR-022 | MET | processChunk: reverb at line 445 processes AFTER delay at lines 378-440. Test: "FR-022: reverb processes delay output" -- difference=0.563 between delay+reverb vs reverb-only chains. |
...
| FR-024 | MET | `ruinae_effects_chain.h` lines 106-116: all 5 delays + freeze + reverb prepared with appropriate per-type API signatures, normalized into single `prepare()`. |
...
| SC-001 | MET | Test: "SC-001: CPU benchmark" -- Digital delay + reverb active, 1000 blocks x 512 samples at 44.1kHz. Measured: **1.45% CPU** (168.5ms wall / 11610ms audio). Spec target: <3%. Regression guard: 10%. |
| SC-002 | MET | Test: "SC-002: crossfade produces no discontinuities" -- (1) ClickDetector with 500ms delay, Digital->PingPong: **0 clicks** (sigma=5.0 threshold). (2) DC signal, 50ms delay, 5-block crossfade window: worst step **-74.47 dBFS** (0.000189 linear). Spec target: < -60 dBFS. |
...
| SC-005 | MET | 38 test cases cover all 29 FRs. Direct FR-named tests: FR-004/005/006/009/010/011/012/013/014/015/016/017/018(x2)/019/020/021/022/023/026/027/028. Plus: SC-001/002/006/007/008 benchmarks, US6 bypass tests, enum tests, latency comp test, different-outputs test, freeze pitch/shimmer/decay tests. |
...
| SC-008 | MET | Test: "SC-008: 10 consecutive type switches click-free" -- ClickDetector with 500ms delay, sigma=5.0 threshold, cycling Digital->Tape->PingPong->Granular->Spectral twice: **0 clicks** detected over 20480 samples. |
