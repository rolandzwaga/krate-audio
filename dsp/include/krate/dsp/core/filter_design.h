...
[[nodiscard]] inline float combFeedbackForRT60(float delayMs, float rt60Seconds) noexcept {
    ...
    // 10^x = e^(x * ln(10))
    return Krate::DSP::detail::constexprExp(exponent * detail::kLn10);
}
...
[[nodiscard]] inline float chebyshevQ(size_t stage, size_t numStages, float rippleDb) noexcept {
    ...
    // Filter order = 2 * numStages (each biquad is a 2nd-order section)
    const size_t filterOrder = numStages * 2;
    const float n = static_cast<float>(filterOrder);

    // Fall back to Butterworth for zero or negative ripple
    if (rippleDb <= 0.0f) {
        // Butterworth Q: 1 / (2 * cos(pi * (2*k + 1) / (4*n)))
        // where n is the number of biquad stages
        const float nStages = static_cast<float>(numStages);
        const float k = static_cast<float>(stage);
        const float angle = kPi * (2.0f * k + 1.0f) / (4.0f * nStages);
    ...
    // mu = (1/n) * asinh(1/epsilon) where n is the filter order
    const float mu = (1.0f / n) * std::asinh(1.0f / epsilon);

    // For Chebyshev, biquads are formed from conjugate pole pairs.
    // Poles are symmetric about the real axis. For each biquad stage,
    // we use a pair of complex conjugate poles.
    //
    // For filter order n, the k-th pole (0 <= k < n) is at angle:
    //   theta_k = pi * (2*k + 1) / (2*n)
    //
    // For n=4: theta_0=pi/8, theta_1=3*pi/8, theta_2=5*pi/8, theta_3=7*pi/8
    //
    // Q = |pole| / (2 * |sigma|) where sigma = -sinh(mu) * sin(theta)
    // Smaller theta = smaller sin(theta) = smaller |sigma| = HIGHER Q
    //
    // So stage 0 (highest Q) should use the smallest theta (pole index 0)
    // Stage 1 (lower Q) should use the next theta (pole index 1)
    const float theta = kPi * (2.0f * static_cast<float>(stage) + 1.0f) / (2.0f * n);
...
/// float theta0 = FilterDesign::butterworthPoleAngle(0, 2);  // pi/4 * (2*0+1) = pi/4 = 0.785
/// float theta1 = FilterDesign::butterworthPoleAngle(1, 2);  // pi/4 * (2*1+1) = 3pi/4 = 2.356
/// ```
