...
public:
    ...
    void prepare(double sampleRate, const PrepareConfig& config) noexcept {
        ...
        // M2 is regenerated by prepare() AND BY NOTHING ELSE that this engine
        // does on its own - setSeed stores the new seed for the NEXT prepare but
        // never re-runs Gram-Schmidt (FR-021, FR-073, Edge case 23). The ONLY other
        // path is the caller-driven, opt-in rebuildMatrixFromSeed() below
        // (Vorago Phase 12, spec B-4); Seraphis never calls it.
    ...
    /// PrepareConfig::seed - or what rebuildMatrixFromSeed() rebuilds from, when a
    /// caller opts in to a live rebuild (Vorago Phase 12, spec B-4).
    ///
    ...
    /// @brief OPT-IN live rebuild of the Dimensionality matrix endpoint M2 from
    ///        the stored seed (Vorago Phase 12, spec B-4 / FR-023).
    ///
    /// setSeed() deliberately leaves M2 alone (FR-021 / FR-073 above). A caller
    /// whose seed is a live parameter - Vorago's CavernVerb - needs a seed
    /// change to reproduce a fresh instance prepared at that seed, which is
    /// impossible while M2 still encodes the old one. This method re-runs
    /// MatrixMorph::build() on the stored seed and forces one re-materialisation
    /// of matrix_ at the next control chunk (the same "-1" idiom reset() uses).
    ///
    /// Nothing calls it inside this header, so every existing consumer
    /// (Seraphis) keeps prepare()-only semantics bit for bit.
    ///
    /// @note Real-time safe. MatrixMorph::build() works on member storage and
    ///       fixed-size stack scratch (kOrder^2 doubles); it allocates nothing.
    ///       A no-op before prepare() - prepare() builds M2 itself.
    void rebuildMatrixFromSeed() noexcept {
        if (!prepared_) {
            return;
        }
        morph_.build(numChannels_, deriveStreamSeed(seed_, kMatrixSalt));
