...
for (const group of dispatch.groups) {
  ...
  // the phase's primary target only; the final group always runs the full set
  // (that is where SC-013's no-regression check lives).
  // 2026-09-10: the previous rule treated ANY dsp/ path as wide, so every gate of
  // a DSP-library phase ran all 8 suites (~12 min each, 20 gates).
  const touchesWide = group.tasks.some(t => (t.files || []).some(f => {
    ...
    return /dsp\/include\/krate\/dsp\/(core|primitives|processors)\//.test(p)
      ...
      || p.includes('CMakeLists.txt') || p.includes('lint_all_headers')
  }))
  const primaryTargets = dispatch.test_targets.filter(t => /vorago/i.test(t))
  ...
  while (attempt < 4) {
    ...
    const fixNote = await run(
      `${CONTEXT}\n\nThe build/tests are failing after implementing group "${group.name}" of ${TASKS}. Diagnose and fix the ROOT CAUSE. Rules: never weaken or delete a failing test to make it pass unless it demonstrably contradicts ${SPEC} (then say so loudly in your notes); no warnings allowed; fix all failures, not just the first. NOT failures (do not "fix" these, just note them): targets a LATER tasks.md group creates that don't exist yet, failing cases a tasks.md section explicitly documents as expected-red until a later task lands, and NEW test TUs that do not compile because tasks.md says their member lands in a later task (say so in one line and stop; do not implement the later task early). You MAY build/run tests yourself while iterating (always to a log file under F:/tmp, tail it, never find /c or any disk-wide walk).${BUILD_CMDS(gateTargets)}\n\nFAILURES:\n${buildLog.failures}\n\nTASK NOTES SO FAR:\n${JSON.stringify(implResults, null, 2)}`,
