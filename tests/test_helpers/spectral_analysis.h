// ==============================================================================
// Test Helper: Spectral Analysis Utilities
// ==============================================================================
// FFT-based aliasing measurement for quantitative verification of anti-aliasing
// success criteria (e.g., SC-001/SC-002 for ADAA specs).
//
...
[[nodiscard]] inline float calculateAliasedFrequency(
    ...
    // No aliasing if below or equal to Nyquist
    if (harmonicFreq <= nyquist) {
        return harmonicFreq;
    }

    // Fold back around Nyquist
    // Formula: f_aliased = |k * fs - harmonicFreq| where k chosen so result is in [0, nyquist]
...
/// @param sampleRate Sample rate in Hz
/// @return true if harmonic frequency exceeds Nyquist (>= for Nyquist edge case)
[[nodiscard]] inline bool willAlias(
    float fundamentalHz,
    int harmonicNumber,
    float sampleRate
) noexcept {
    // Note: harmonics exactly AT Nyquist are categorized as aliased per spec
    return (fundamentalHz * static_cast<float>(harmonicNumber)) > (sampleRate / 2.0f);
}

// =============================================================================
// Data Structures (User Story 2)
// =============================================================================
...
// =============================================================================
// Bin Collection Functions (User Story 2)
// =============================================================================

/// @brief Get bin indices for intended harmonics (strictly below Nyquist)
...
/// @note Harmonics exactly at Nyquist (freq >= nyquist) are categorized as aliased
...
/// @return dB value (returns kFloorDb for very small values)
...
/// @brief Sum power from specified bins (RMS calculation)
/// @param spectrum Pointer to FFT spectrum
/// @param bins Vector of bin indices to sum
/// @return RMS power (sqrt of sum of squared magnitudes)
...
// =============================================================================
// Result Structures (User Story 3)
// =============================================================================
...
struct AliasingMeasurement {
    float fundamentalPowerDb = 0.0f;    ///< Power at fundamental frequency (dB)
    float harmonicPowerDb = 0.0f;       ///< Total power in intended harmonics below Nyquist (dB)
    float aliasingPowerDb = 0.0f;       ///< Total power in aliased components (dB)
    float signalToAliasingDb = 0.0f;    ///< Fundamental minus aliasing (dB), higher = better
...
// =============================================================================
// Main Measurement Functions (User Story 3)
// =============================================================================
...
// =============================================================================
// Comparison Results (User Story 4)
// =============================================================================

/// @brief Result of comparing two waveshapers' aliasing characteristics
///
/// This structure holds the comparison result between a reference waveshaper
/// (typically naive hard clip) and a tested waveshaper (e.g., ADAA).
///
/// @par Example Usage
/// @code
/// auto result = compareAliasing(config, hardClipReference,
///                               [](float x) { return adaa.process(x); });
/// REQUIRE(result.meetsThreshold(12.0f));  // SC-001: at least 12dB improvement
/// @endcode
struct AliasingComparison {
    float referenceAliasing = 0.0f;  ///< Aliasing power of reference (dB)
    float testedAliasing = 0.0f;     ///< Aliasing power of tested processor (dB)
    float reductionDb = 0.0f;        ///< Aliasing reduction (positive = improvement)

    /// @brief Check if reduction meets a threshold requirement
    /// @param thresholdDb Required aliasing reduction in dB
    /// @return true if reductionDb >= thresholdDb
    [[nodiscard]] bool meetsThreshold(float thresholdDb) const noexcept {
        return reductionDb >= thresholdDb;
    }
};

// =============================================================================
// Reference Processors (User Story 4)
// =============================================================================

/// @brief Naive hard clip reference processor (y = clamp(x, -1, 1))
/// @param x Input sample
/// @return Hard-clipped output
[[nodiscard]] inline float hardClipReference(float x) noexcept {
    if (x > 1.0f) return 1.0f;
    if (x < -1.0f) return -1.0f;
    return x;
}

/// @brief Identity reference processor (y = x)
/// @param x Input sample
/// @return Input unchanged
[[nodiscard]] inline float identityReference(float x) noexcept {
    return x;
}

// =============================================================================
// Comparison Functions (User Story 4)
// =============================================================================

/// @brief Compare aliasing between two waveshapers
///
/// Measures aliasing for both the reference and tested waveshaper, then
/// computes the difference. Useful for verifying success criteria like
/// "ADAA must reduce aliasing by at least 12dB vs naive hard clip."
///
/// @tparam ReferenceProcessor Callable with signature: float(float)
/// @tparam TestedProcessor Callable with signature: float(float)
/// @param config Test configuration
/// @param reference Reference waveshaper (typically hardClipReference)
/// @param tested Waveshaper being tested
/// @return Comparison result with aliasing reduction
///
/// @par Example
/// @code
...
///     config,
///     hardClipReference,
///     [](float x) { return myADAA.process(x); }
/// );
/// REQUIRE(result.meetsThreshold(12.0f));  // At least 12 dB improvement
/// @endcode
template<typename ReferenceProcessor, typename TestedProcessor>
[[nodiscard]] AliasingComparison compareAliasing(
    const AliasingTestConfig& config,
    ReferenceProcessor&& reference,
    TestedProcessor&& tested
) {
    const auto refResult = measureAliasing(config, std::forward<ReferenceProcessor>(reference));
    const auto testResult = measureAliasing(config, std::forward<TestedProcessor>(tested));

    return AliasingComparison{
        .referenceAliasing = refResult.aliasingPowerDb,
        .testedAliasing = testResult.aliasingPowerDb,
        .reductionDb = testResult.aliasingReductionVs(refResult)
    };
}
