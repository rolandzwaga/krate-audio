...
// glide must also fire on the SLOW path (per-sample FeedbackExciter). Before
// the slow-path completion fix, processBlockSlow had no tension code at all,
// so the FeedbackExciter route produced a static pitch regardless of velocity
// (highVelF0 == lowVelF0) and this CHECK fails. Mirrors the fast-path test
// above but swaps the exciter to Feedback, which forces useSlowPath.
TEST_CASE("Phase 8E: tension glide also fires on the slow (Feedback) path",
