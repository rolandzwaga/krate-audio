...
void PadGridView::draw(VSTGUI::CDrawContext* ctx)
{
    ...
    for (int row = 0; row < kPadGridRows; ++row)
    {
        for (int col = 0; col < kPadGridColumns; ++col)
        {
            ...
            char label[16] = {};
            std::snprintf(label, sizeof(label), "%d", midi);
            ctx->setFontColor(kLabelColor);

            VSTGUI::CRect labelRect = inset;
            labelRect.bottom = labelRect.top + 14.0;
            ctx->drawString(label, labelRect, VSTGUI::kCenterText, true);
