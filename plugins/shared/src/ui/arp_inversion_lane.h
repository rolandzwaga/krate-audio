...
// ==============================================================================
// Custom CControl implementing IArpLane that renders per-step inversion cells.
// Left-click opens COptionMenu popup with 4 inversion types, right-click resets
// to Root (index 0). Follows ArpConditionLane pattern.
//
...
class ArpInversionLane : public VSTGUI::CControl, public IArpLane {
public:
    // =========================================================================
    // Constants
    // =========================================================================
    ...
    static constexpr int kInversionCount = 4;
    static constexpr float kBodyHeight = 47.0f;
    static constexpr float kLeftMargin = 40.0f;
    static constexpr float kNormDivisor = 3.0f;  // kInversionCount - 1

    // Abbreviated labels for cell display
    static constexpr const char* kInversionAbbrev[4] = {
        "Rt", "1st", "2nd", "3rd"
    };

    // Full names for COptionMenu popup entries
    static constexpr const char* kInversionFullNames[4] = {
    ...
    ArpInversionLane(const VSTGUI::CRect& size,
        ...
        // stepValues_ is zero-initialized = all Root (0)
    }
    ...
    void setStepValue(int index, uint8_t value) {
        if (index >= 0 && index < kMaxSteps) {
            if (value >= kInversionCount) value = 0;
    ...
    [[nodiscard]] int32_t getLaneTypeId() const override {
        return 7;  // ClipboardLaneType::kInversion
    }
    ...
    [[nodiscard]] std::array<float, 32> computeTransform(TransformType type) const {
        ...
        switch (type) {
            case TransformType::kInvert:
                // Invert: reverse the inversion index (Root<->3rd, 1st<->2nd)
                for (int32_t i = 0; i < len; ++i) {
                    auto idx = static_cast<int>(
                        std::round(result[static_cast<size_t>(i)] * kNormDivisor));
                    idx = (kInversionCount - 1) - idx;
            ...
            case TransformType::kRandomize: {
                ...
                std::mt19937 rng(rd());
                std::uniform_int_distribution<int> dist(0, kInversionCount - 1);
                for (int32_t i = 0; i < len; ++i)
                    result[static_cast<size_t>(i)] =
    ...
    VSTGUI::CMouseEventResult onMouseDown(
        ...
        for (int i = 0; i < kInversionCount; ++i)
            menu->addEntry(kInversionFullNames[i]);

        uint8_t currentVal = stepValues_[static_cast<size_t>(step)];
        if (currentVal < kInversionCount)
        ...
        if (selectedIndex >= 0 && selectedIndex < kInversionCount) {
            if (beginEditCallback_ && stepBaseParamId_ != 0)
                beginEditCallback_(stepBaseParamId_ + static_cast<uint32_t>(step));
...
private:
    ...
    void drawBody(VSTGUI::CDrawContext* context, const VSTGUI::CRect& vs) {
        ...
        for (int i = 0; i < numSteps_; ++i) {
            ...
            context->drawString(VSTGUI::UTF8String(kInversionAbbrev[val]),
...
struct ArpInversionLaneCreator : VSTGUI::ViewCreatorAdapter {
    ArpInversionLaneCreator() {
        VSTGUI::UIViewFactory::registerViewCreator(*this);
    }

    VSTGUI::IdStringPtr getViewName() const override { return "ArpInversionLane"; }
    ...
    VSTGUI::UTF8StringPtr getDisplayName() const override {
        return "Arp Inversion Lane";
    }

    VSTGUI::CView* create(
        const VSTGUI::UIAttributes& /*attributes*/,
        const VSTGUI::IUIDescription* /*description*/) const override {
        return new ArpInversionLane(VSTGUI::CRect(0, 0, 500, 44), nullptr, -1);
    }

    bool apply(VSTGUI::CView* view, const VSTGUI::UIAttributes& attributes,
               const VSTGUI::IUIDescription* description) const override {
        auto* lane = dynamic_cast<ArpInversionLane*>(view);
