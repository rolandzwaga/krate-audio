...
inline bool loadDelayParamsV9(RuinaeDelayParams& params, Steinberg::IBStreamer& streamer) {
    ...
    // Digital
    if (!streamer.readInt32(iv)) return false; params.digitalEra.store(iv, std::memory_order_relaxed);
    if (!streamer.readFloat(fv)) return false; params.digitalAge.store(fv, std::memory_order_relaxed);
    if (!streamer.readInt32(iv)) return false; params.digitalLimiter.store(iv, std::memory_order_relaxed);
    if (!streamer.readFloat(fv)) return false; params.digitalModDepth.store(fv, std::memory_order_relaxed);
    if (!streamer.readFloat(fv)) return false; params.digitalModRateHz.store(fv, std::memory_order_relaxed);
    if (!streamer.readInt32(iv)) return false; params.digitalModWaveform.store(iv, std::memory_order_relaxed);
    if (!streamer.readFloat(fv)) return false; params.digitalWidth.store(fv, std::memory_order_relaxed);
    if (!streamer.readFloat(fv)) return false; params.digitalWavefoldAmt.store(fv, std::memory_order_relaxed);
    if (!streamer.readInt32(iv)) return false; params.digitalWavefoldModel.store(iv, std::memory_order_relaxed);
    if (!streamer.readFloat(fv)) return false; params.digitalWavefoldSym.store(fv, std::memory_order_relaxed);

    // Tape
    if (!streamer.readFloat(fv)) return false; params.tapeInertiaMs.store(fv, std::memory_order_relaxed);
    if (!streamer.readFloat(fv)) return false; params.tapeWear.store(fv, std::memory_order_relaxed);
    if (!streamer.readFloat(fv)) return false; params.tapeSaturation.store(fv, std::memory_order_relaxed);
    if (!streamer.readFloat(fv)) return false; params.tapeAge.store(fv, std::memory_order_relaxed);
    if (!streamer.readInt32(iv)) return false; params.tapeSpliceEnabled.store(iv != 0, std::memory_order_relaxed);
    if (!streamer.readFloat(fv)) return false; params.tapeSpliceIntensity.store(fv, std::memory_order_relaxed);
    if (!streamer.readInt32(iv)) return false; params.tapeHead1Enabled.store(iv != 0, std::memory_order_relaxed);
    if (!streamer.readFloat(fv)) return false; params.tapeHead1Level.store(fv, std::memory_order_relaxed);
    if (!streamer.readFloat(fv)) return false; params.tapeHead1Pan.store(fv, std::memory_order_relaxed);
    if (!streamer.readInt32(iv)) return false; params.tapeHead2Enabled.store(iv != 0, std::memory_order_relaxed);
    if (!streamer.readFloat(fv)) return false; params.tapeHead2Level.store(fv, std::memory_order_relaxed);
    if (!streamer.readFloat(fv)) return false; params.tapeHead2Pan.store(fv, std::memory_order_relaxed);
    if (!streamer.readInt32(iv)) return false; params.tapeHead3Enabled.store(iv != 0, std::memory_order_relaxed);
    if (!streamer.readFloat(fv)) return false; params.tapeHead3Level.store(fv, std::memory_order_relaxed);
    if (!streamer.readFloat(fv)) return false; params.tapeHead3Pan.store(fv, std::memory_order_relaxed);

    // Granular
    if (!streamer.readFloat(fv)) return false; params.granularSizeMs.store(fv, std::memory_order_relaxed);
    if (!streamer.readFloat(fv)) return false; params.granularDensity.store(fv, std::memory_order_relaxed);
    if (!streamer.readFloat(fv)) return false; params.granularPitch.store(fv, std::memory_order_relaxed);
    if (!streamer.readFloat(fv)) return false; params.granularPitchSpray.store(fv, std::memory_order_relaxed);
    if (!streamer.readInt32(iv)) return false; params.granularPitchQuant.store(iv, std::memory_order_relaxed);
    if (!streamer.readFloat(fv)) return false; params.granularPosSpray.store(fv, std::memory_order_relaxed);
    if (!streamer.readFloat(fv)) return false; params.granularReverseProb.store(fv, std::memory_order_relaxed);
    if (!streamer.readFloat(fv)) return false; params.granularPanSpray.store(fv, std::memory_order_relaxed);
    if (!streamer.readFloat(fv)) return false; params.granularJitter.store(fv, std::memory_order_relaxed);
    if (!streamer.readFloat(fv)) return false; params.granularTexture.store(fv, std::memory_order_relaxed);
    if (!streamer.readFloat(fv)) return false; params.granularWidth.store(fv, std::memory_order_relaxed);
    if (!streamer.readInt32(iv)) return false; params.granularEnvelope.store(iv, std::memory_order_relaxed);
    if (!streamer.readInt32(iv)) return false; params.granularFreeze.store(iv != 0, std::memory_order_relaxed);

    // Spectral
    if (!streamer.readInt32(iv)) return false; params.spectralFFTSize.store(iv, std::memory_order_relaxed);
    if (!streamer.readFloat(fv)) return false; params.spectralSpreadMs.store(fv, std::memory_order_relaxed);
    if (!streamer.readInt32(iv)) return false; params.spectralDirection.store(iv, std::memory_order_relaxed);
    if (!streamer.readInt32(iv)) return false; params.spectralCurve.store(iv, std::memory_order_relaxed);
    if (!streamer.readFloat(fv)) return false; params.spectralTilt.store(fv, std::memory_order_relaxed);
    if (!streamer.readFloat(fv)) return false; params.spectralDiffusion.store(fv, std::memory_order_relaxed);
    if (!streamer.readFloat(fv)) return false; params.spectralWidth.store(fv, std::memory_order_relaxed);
    if (!streamer.readInt32(iv)) return false; params.spectralFreeze.store(iv != 0, std::memory_order_relaxed);

    // PingPong
    if (!streamer.readInt32(iv)) return false; params.pingPongRatio.store(iv, std::memory_order_relaxed);
    if (!streamer.readFloat(fv)) return false; params.pingPongCrossFeed.store(fv, std::memory_order_relaxed);
    if (!streamer.readFloat(fv)) return false; params.pingPongWidth.store(fv, std::memory_order_relaxed);
    if (!streamer.readFloat(fv)) return false; params.pingPongModDepth.store(fv, std::memory_order_relaxed);
    if (!streamer.readFloat(fv)) return false; params.pingPongModRateHz.store(fv, std::memory_order_relaxed);
