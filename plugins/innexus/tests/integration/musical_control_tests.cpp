...
// =============================================================================
// T096: Loading v3 state applies M4 defaults
// =============================================================================
TEST_CASE("M4 State: loading v3 state applies M4 defaults (backward compat)",
          "[innexus][m4][state][backward]")
{
    M4TestStream stream;

    // Create a v3 state (without M4 data) using an old-format writer
    // We'll use a processor with M3 parameters but write version 3
    // The simplest approach: save state normally (v4), but we actually need
    // a v3 state. Let's manually construct a minimal v3 state by saving from
    // a fresh processor (defaults) and verifying that when we load it
    // the M4 defaults are applied.
    //
    // Better approach: set non-default M4 values on one processor, save v4,
    // then create a second processor, set it to save as v3 by not having
    // M4 code... but that's not possible since the code is built in.
    //
    // Best approach: save default state (which writes v4), then manually
    // create a v3 state by writing v3 format ourselves.
    {
        // Manually write a v3 state blob
        ...
        // NO M4 data -- this is a v3 state
    }

    // Load into a processor and verify M4 defaults
    stream.resetReadPos();
    Innexus::Processor proc;
    proc.initialize(nullptr);
    auto setup = makeSetup();
    ...
    // Verify M4 defaults
    REQUIRE(proc.getFreeze() == Approx(0.0f).margin(0.01f));           // freeze off
    REQUIRE(proc.getResponsiveness() == Approx(0.5f).margin(0.01f));   // default 0.5
    REQUIRE(proc.getCurrentFilterType() == 0);                          // All-Pass

    // Morph position defaults to 0.0 -- verify via get state round-trip
    // Save state and reload to verify morph position
    M4TestStream verifyStream;
    REQUIRE(proc.getState(&verifyStream) == kResultOk);
    verifyStream.resetReadPos();

    Innexus::Processor verifier;
    verifier.initialize(nullptr);
    auto setup2 = makeSetup();
    verifier.setupProcessing(setup2);
    verifier.setActive(true);
    REQUIRE(verifier.setState(&verifyStream) == kResultOk);

    // All M4 defaults should still be defaults after round-trip
    REQUIRE(verifier.getFreeze() == Approx(0.0f).margin(0.01f));
    REQUIRE(verifier.getResponsiveness() == Approx(0.5f).margin(0.01f));
    REQUIRE(verifier.getCurrentFilterType() == 0);
